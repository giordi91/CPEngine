...
private:
  void set32BitMem(unsigned char*ptr, int sizeInBtye, int value) {
...
public:
  ...
  void initialize(const uint32_t totalSizeInByte,
    ...
    m_memory = new unsigned char[totalSizeInByte];
  ...
  inline unsigned char *getPointer(const RandomSizeAllocationHandle handle) const {
  ...
  inline void tagMemoryAsFreed(const RandomSizeAllocationHandle handle) {
    unsigned char *ptr = getPointer(handle);
    assert((reinterpret_cast<int *>(ptr)[0] != DEBUG_VALUE));
	set32BitMem(ptr,handle.allocSize,DEBUG_VALUE);
  }

  inline const unsigned char *getStartPtr() const { return m_memory; };
  inline const unsigned char *getUnfragmentedPtr() const { return m_unfragmentedPtr; };
...
private:
  ...
  unsigned char *m_unfragmentedPtr = nullptr;
