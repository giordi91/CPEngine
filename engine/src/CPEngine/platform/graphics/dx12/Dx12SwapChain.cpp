...
extern DXGI_FORMAT BACKBUFFER_FORMAT = DXGI_FORMAT_R8G8B8A8_UNORM;
extern DXGI_FORMAT DEPTHSTENCIL_FORMAT = DXGI_FORMAT_D24_UNORM_S8_UINT;
...
ID3D12Resource *createDepthTexture(D3D12DeviceType *device, const char *name,
  ...
  depthStencilDesc.SampleDesc.Quality = msaaState ? (m_msaaQuality - 1) : 0;
...
bool createSwapchain(Dx12SwapChain *swapchain,
  ...
  const Dx12Resources *resources = context->getResourcesReadOnly();

  const auto &settings = context->getContextSettings();
  swapchain->m_inFlightFrames = settings.inFlightFrames;
  ...
  swapDesc.SampleDesc.Count = msaaState ? 4 : 1;
  swapDesc.SampleDesc.Quality = msaaState ? msaaQuality - 1 : 0;
  ...
  // the queue
  result = context->getFactory()->CreateSwapChain(
      resources->globalCommandQueue, &swapDesc, &swapchain->swapChain);
...
bool resizeSwapchain(Dx12RenderingContext *context, Dx12SwapChain *swapchain,
  ...
  context->flushGlobalCommandQueue();
  ...
  const RenderingContextCreationSettings &settings =
      context->getContextSettings();
  for (uint32_t i = 0; i < settings.inFlightFrames; ++i) {
    // dx12::TEXTURE_MANAGER->free(m_swapChainBuffersHandles[i]);
    if (swapchain->m_swapChainBuffersHandles[i] != nullptr) {
      swapchain->m_swapChainBuffersHandles[i]->Release();
    //TODO FREE DESCRIPTORS
    }
  }

  // Resize the swap chain.
  result = swapchain->swapChain->ResizeBuffers(
      settings.inFlightFrames, width, height, BACKBUFFER_FORMAT,
      DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH);
  assert(SUCCEEDED(result) && "failed to resize swap chain");

  // resetting the current back buffer
  swapchain->m_currentBackBuffer = 0;
  ...
  for (UINT i = 0; i < settings.inFlightFrames; i++) {
    ...
    swapchain->m_swapChainBuffersHandles[i] = resource;

    swapchain->m_swapChainBuffersDescriptors[i] = initializeFromResourceDx12(
        resources->device, resource, "", D3D12_RESOURCE_STATE_COMMON,
        resources->rtvHeap);
  }

  // freeing depth and re-creating it;
  if (swapchain->m_swapChainDepth != nullptr) {
    swapchain->m_swapChainDepth->Release();
    //TODO FREE DESCRIPTORS
  }

  swapchain->m_swapChainDepth = createDepthTexture(resources->device, "depthBuffer", width,
                                        height, D3D12_RESOURCE_STATE_COMMON);
  createDSV(resources->device, resources->dsvHeap, swapchain->m_swapChainDepth,
            swapchain->m_swapChainDepthDescriptors, DXGI_FORMAT_D32_FLOAT_S8X24_UINT);
  ...
  barrier[0] = CD3DX12_RESOURCE_BARRIER::Transition(
      swapchain->m_swapChainDepth, D3D12_RESOURCE_STATE_COMMON,
  ...
  swapchain->m_screenViewport.TopLeftX = 0;
  swapchain->m_screenViewport.TopLeftY = 0;
  swapchain->m_screenViewport.Width = static_cast<float>(width);
  swapchain->m_screenViewport.Height = static_cast<float>(height);
  swapchain->m_screenViewport.MinDepth = 0.0f;
  swapchain->m_screenViewport.MaxDepth = 1.0f;

  swapchain->m_scissorRect = {0, 0, width, height};
  return true;
}
